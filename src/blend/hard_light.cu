...
__device__ __forceinline__ float hard_light_channel(float b, float s) {
    if (s < 0.5f) {
        return 2.0f * s * b;
    } else {
        return 1.0f - 2.0f * (1.0f - s) * (1.0f - b);
    }
}

__global__ void blend_hard_light_kernel(
    ...
    const float blend_r = hard_light_channel(dst.x, src.x);
    const float blend_g = hard_light_channel(dst.y, src.y);
    const float blend_b = hard_light_channel(dst.z, src.z);
