...
__device__ __forceinline__ float overlay_channel(float b, float s) {
    if (b < 0.5f) {
        return 2.0f * b * s;
    } else {
        return 1.0f - 2.0f * (1.0f - b) * (1.0f - s);
    }
}

__global__ void blend_overlay_kernel(
    ...
    const float blend_r = overlay_channel(dst.x, src.x);
    const float blend_g = overlay_channel(dst.y, src.y);
    const float blend_b = overlay_channel(dst.z, src.z);
