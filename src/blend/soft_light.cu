...
__device__ __forceinline__ float soft_light_channel(float b, float s) {
    return (1.0f - 2.0f * s) * b * b + 2.0f * s * b;
}

__global__ void blend_soft_light_kernel(
    ...
    const float blend_r = soft_light_channel(dst.x, src.x);
    const float blend_g = soft_light_channel(dst.y, src.y);
    const float blend_b = soft_light_channel(dst.z, src.z);
    ...
    if (out_alpha > 1e-6f) {
        ...
        out_r = (blend_r * src_alpha + dst.x * dst_alpha * (1.0f - src_alpha)) * inv_out_alpha;
        out_g = (blend_g * src_alpha + dst.y * dst_alpha * (1.0f - src_alpha)) * inv_out_alpha;
        out_b = (blend_b * src_alpha + dst.z * dst_alpha * (1.0f - src_alpha)) * inv_out_alpha;
    } else {
