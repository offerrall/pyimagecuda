...
__global__ void blend_normal_kernel(
    ...
    if (out_alpha > 1e-6f) {
        ...
        out_r = (src.x * src_alpha + dst.x * dst_alpha * (1.0f - src_alpha)) * inv_out_alpha;
        out_g = (src.y * src_alpha + dst.y * dst_alpha * (1.0f - src_alpha)) * inv_out_alpha;
        out_b = (src.z * src_alpha + dst.z * dst_alpha * (1.0f - src_alpha)) * inv_out_alpha;
    } else {
        out_r = 0.0f;
        out_g = 0.0f;
        out_b = 0.0f;
